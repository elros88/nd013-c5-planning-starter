...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // ustd::sing a comfortable deceleration.
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // use std::costd::sine and std::sine to get x and y
      //
      ...
      // HINT: _speed_limit * std::std::sin/std::cos (goal.rotation.yaw);
    ...
    // move straight to "STOPPED" state. To solve this issue (std::since we don't
